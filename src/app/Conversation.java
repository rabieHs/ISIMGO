...
public class Conversation extends Thread {
    ...
    public void run() {
        try {
            ...
            printWriter.println("Welcome to ISIMGO App!");
            ...
            switch (messageClient) {
                case "1":
                    ...
                    User authenticatedUser = server.authenticateUser(email, password,socket);
                    if (authenticatedUser != null) {
                        ...
                        printWriter.println("there is no user with this credentials!!");
                        this.run();
                ...
                case "2":
                    ...
                    if (isRegistered) {
                        ...
                        User registredUser = server.authenticateUser(email, password,socket);
    ...
    private void handleUserMenu(PrintWriter printWriter, BufferedReader bufferedReader, User authenticatedUser) throws IOException {
        ...
        switch (userCommand) {
            ...
            case "2":
                ...
                List<Post> allPosts = server.getAllPosts();
            ...
            default:
                printWriter.println("Invalid command!");
                this.handleUserMenu(printWriter, bufferedReader, authenticatedUser); // Stay in the user menu
        }
    ...
    private void startConversation(PrintWriter printWriter, BufferedReader bufferedReader, User authenticatedUser) throws IOException {
        ...
        if (receiver != null) {
            ...
            this.startConversation(printWriter,bufferedReader,authenticatedUser);
        }
    }
    ...
    private void viewPostDetails(PrintWriter printWriter, BufferedReader bufferedReader, int postId,User authenticatedUser) throws IOException {
        ...
        if (post != null) {
            ...
            switch (userCommand) {
                case "1":
                    boolean isLiked= server.addLike(post,authenticatedUser);
                    if(isLiked){
                        printWriter.println("Post Liked Successfully command!");
                    }else{
                        printWriter.println("you already liked this post!");
                    }
                    viewPostDetails(printWriter, bufferedReader, postId, authenticatedUser);
                ...
                case "2":
                    ...
                    server.addComment(post,authenticatedUser,commentContent);
                    viewPostDetails(printWriter, bufferedReader, postId, authenticatedUser);
    ...
    private Post findPostById(int postId) {
        ...
        for (Post post : server.getAllPosts()) {
   ...
   /* private synchronized void commentOnPost(PrintWriter printWriter, BufferedReader bufferedReader, Post post, User user) throws IOException {
        printWriter.println("Enter your comment:");
        String commentContent = bufferedReader.readLine();
    ...
    private void printPostsList(PrintWriter printWriter) {
        printWriter.println("Posts List:");
        List<Post> posts = server.getAllPosts(); // Assuming you have a method to retrieve posts in your Server class
        if (posts.isEmpty()) {
